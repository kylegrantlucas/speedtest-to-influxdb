...
// 	expectDistance := 0
// 	assert.EqualValues(t, servers[1].Distance, expectDistance, fmt.Sprintf("Server 1 name should be: '%v'\n", expectDistance))

// 	expectLatency := 0
// 	assert.EqualValues(t, servers[21].Latency, expectLatency, fmt.Sprintf("Server 21 name should be: '%v'\n", expectLatency))
...
// 	fs, err := stc.GetFastestServer(servers)
// 	if err != nil {
// 		t.Fatalf("couldn't get fastest server: %v", err)
// 	}
